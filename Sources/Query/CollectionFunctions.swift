...
public struct Map: Expr {
    ...
    public init(collection: Expr, lambda: Expr) {
        value = fn(["map": lambda.value, "collection": collection.value])
    }
...
public struct Foreach: Expr {
     ...
     */
    public init(collection: Expr, lambda: Expr){
        value = fn(["foreach": lambda.value, "collection": collection.value])
    }
...
public struct Filter: Expr {
     ...
     */
    public init(collection: Expr, lambda: Expr) {
        value = fn(["filter": lambda.value, "collection": collection.value])
    }
...
public struct Take: Expr {
    ...
    public init (count: Expr, collection: Expr){
        value = fn(["take": count.value, "collection": collection.value])
    }
}
...
public struct Drop: Expr {
     ...
     */
    public init(count: Expr, collection: Expr) {
        value = fn(["drop": count.value, "collection": collection.value])
    }
...
public struct Prepend: Expr {
    ...
    public init(elements: Expr, toCollection collection: Expr){
        value = fn(["collection": elements.value, "prepend": collection.value])
    }
}
...
public struct Append: Expr {
     ...
     */
    public init(elements: Expr, toCollection collection: Expr){
        value = fn(["collection": elements.value, "append": collection.value])
    }
}